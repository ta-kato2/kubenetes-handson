...
public interface LocomocoEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/locomoco */
    ...
    /** The key of the configuration. e.g. returnpath@local.mobile.dev */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. com.mysql.jdbc.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:mysql://localhost:3306/locomocodb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. locomocodb */
     ...
     * The value is, e.g. /tmp/lastaflute/locomoco <br>
     ...
     * The value is, e.g. returnpath@local.mobile.dev <br>
     ...
     * The value is, e.g. jdbc:mysql://localhost:3306/locomocodb <br>
     ...
     * The value is, e.g. locomocodb <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements LocomocoEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(LocomocoEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(LocomocoEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(LocomocoEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(LocomocoEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(LocomocoEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(LocomocoEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(LocomocoEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(LocomocoEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(LocomocoEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(LocomocoEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(LocomocoEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(LocomocoEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(LocomocoEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(LocomocoEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(LocomocoEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(LocomocoEnv.MAIL_RETURN_PATH);
        }

        public String getJdbcDriver() {
            return get(LocomocoEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(LocomocoEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(LocomocoEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(LocomocoEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(LocomocoEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(LocomocoEnv.JDBC_CONNECTION_POOLING_SIZE);
        }
    }
}
